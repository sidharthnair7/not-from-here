...
export interface InfiniteMenuProps {
  ...
  onActionClick?: (item: MenuItem) => void;
...
class InfiniteGridMenu {
  ...
  setScale(newScale: number) {
    ...
    this.camera.position[2] = 3 * newScale;
    if (this.gl) {
      ...
      this.items.map(
        item =>
          new Promise<HTMLImageElement>(resolve => {
            ...
            img.onerror = () => {
              // Create fallback color tile
              const fb = document.createElement('canvas');
              fb.width = cellSize;
              fb.height = cellSize;
              const fbc = fb.getContext('2d');
              if (fbc) {
                fbc.fillStyle = '#101724';
                fbc.fillRect(0, 0, cellSize, cellSize);
                fbc.fillStyle = '#62d59a';
                fbc.font = '600 32px sans-serif';
                fbc.textAlign = 'center';
                fbc.fillText(item.title || 'Specimen', cellSize / 2, cellSize / 2);
              }
  ...
  #updateProjectionMatrix(_gl: WebGL2RenderingContext) {
...
const defaultItems: MenuItem[] = [
  {
    ...
    title: 'Specimen',
    description: 'Verified sighting record'
  }
];
...
export const InfiniteMenu: React.FC<InfiniteMenuProps> = ({
  ...
  // Sync active item with parent if callback provided
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const effectiveItems = items.length ? items : defaultItems;

    const handleActiveItem = (index: number) => {
      const itemIndex = index % effectiveItems.length;
  ...
  return (
    <div
      className={`infinite-menu-container ${className}`.trim()}
      style={
        {
          backgroundColor,
